...
<body class="font-light antialiased bg-[#f0f4f8]">
    ...
    <div class="relative w-full" style="max-width: 100%; height: 400px;">
        ...
        <img src="{{ asset('storage/images/A_horizontal_image_related_to_the_IT_industry,_fea.jpg') }}"
            alt="ProSnap Image" class="w-full h-full object-cover">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-50 text-white p-6">
            ...
            <p class="text-lg md:text-xl mt-4">Find it difficult to find a job? Post your resume NOW and get a job in
                less than 24 hours!!!</p>
            <a href="{{ route('make_post.index') }}"
                class="mt-6 bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-2 px-4 rounded-lg">Post your
                Resume</a>
        </div>
    ...
    <footer
        ...
        <a class="hover:text-yellow-400 text-sm text-white" href="{{ route('employer.register') }}">
            Employer Register
        </a>
        <a class="hover:text-yellow-400 text-sm text-white" href="{{ route('employer.login') }}">
            Employer Login
        </a>
    </footer>
</body>