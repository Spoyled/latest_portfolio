...
<div class="w-full text-center py-32">
  <h1 class="text-2xl md:text-3xl font-bold text-center lg:text-5xl text-yellow-900">
    {{ $post->name }}'s <span class="text-black">post - </span>
    <span class="text-yellow-600">{{ $post->title }}</span>
  </h1>
</div>
    ...
    <div class="mb-16">
      <div class="flex justify-center items-center">
        <img class="object-cover h-96 w-96 rounded-full shadow-lg"
             src="{{ $post->image ? asset('storage/posts/'.$post->image) : 'https://www.pngall.com/wp-content/uploads/12/Avatar-Profile-Vector.png' }}"
             alt="Post Image">
      </div>
        ...
        <div class="flex flex-col space-y-2 mt-4">
          @if(!empty($post->additional_links))
            <a href="{{ $post->additional_links }}"
               class="bg-blue-500 text-white font-bold py-2 px-4 w-40 rounded hover:bg-blue-700 transition-colors">
              Additional Link
            </a>
          @endif
          ...
          @if(auth()->check() && auth()->id() === ($post->user_id ?? null))
            <div class="text-center mt-4">
              <a href="{{ route('posts.edit', $post->id) }}"
                 class="inline-block bg-yellow-500 text-white py-2 px-4 rounded-lg font-bold hover:bg-yellow-600">
                Edit Post
              </a>
            </div>
          @endif
      ...
      <div class="comment p-4 bg-gray-100 rounded-lg" id="comment-{{ $comment->id }}">
        ...
        @if(Auth::check() && Auth::user()->is_admin)
          <form action="{{ route('comments.destroy', $comment->id) }}" method="POST"
            ...
            @csrf @method('DELETE')
            <button type="button" class="text-red-500 font-bold hover:text-red-700 delete-comment-btn">Delete</button>
          </form>
        @endif
      </div>
    @endforeach
